...
# Objective: run InsilicoVA on COMSA data, after mapping variables, and produce CODs for 0-1m, 1-59m, 5-14y, 15-49y, 50+
...
COMSAdata$id10260a <- ifelse(COMSAdata$id10260 %in% c("right_side"), "Y", ifelse(COMSAdata$id10260 %!in% c("dk","DK","ref",".",""), "N", "."))
COMSAdata$id10260b <- ifelse(COMSAdata$id10260 %in% c("left_side"), "Y", ifelse(COMSAdata$id10260 %!in% c("dk","DK","ref",".",""), "N", "."))
COMSAdata$id10260c <- ifelse(COMSAdata$id10260 %in% c("lower_part_of_body"), "Y", ifelse(COMSAdata$id10260 %!in% c("dk","DK","ref",".",""), "N", "."))
COMSAdata$id10260d <- ifelse(COMSAdata$id10260 %in% c("upper_part_of_body"), "Y", ifelse(COMSAdata$id10260 %!in% c("dk","DK","ref",".",""), "N", "."))
COMSAdata$id10260e <- ifelse(COMSAdata$id10260 %in% c("one_leg_only"), "Y", ifelse(COMSAdata$id10260 %!in% c("dk","DK","ref",".",""), "N", "."))
COMSAdata$id10260f <- ifelse(COMSAdata$id10260 %in% c("one_arm_only"), "Y", ifelse(COMSAdata$id10260 %!in% c("dk","DK","ref",".",""), "N", "."))
...
COMSAdata$group[COMSAdata$ageatdeath >= 28 & COMSAdata$ageatdeath < 60*30.4] <- "ch1_59m"
COMSAdata$group[COMSAdata$ageatdeath >= 60*30.4 & COMSAdata$ageatdeath < 15*365.25] <- "ch5_14y"
...
IV5data.ch1_59m <- subset(COMSAdata, group=="ch1_59m")
dim(IV5data.ch1_59m)
IV5data.ch5_14y <- subset(COMSAdata, group=="ch5_14y")
dim(IV5data.ch5_14y)
...
IV5data.ch1_59m.subset <- IV5data.ch1_59m[, c(names.use)]
IV5data.ch5_14y.subset <- IV5data.ch5_14y[, c(names.use)]
...
IV5data.ch1_59m <- setcolorder(IV5data.ch1_59m.subset, names(sample_test))
IV5data.ch5_14y <- setcolorder(IV5data.ch5_14y.subset, names(sample_test))
...
set.seed(123)
codeVAInsilico.ch1_59m <- codeVA(IV5data.ch1_59m, data.type = "WHO2016", model = "InSilicoVA",
                                Nsim=10000, version = "5.0", HIV = "h", Malaria = "h")
set.seed(123)
codeVAInsilico.ch5_14y <- codeVA(IV5data.ch5_14y, data.type = "WHO2016", model = "InSilicoVA",
                                Nsim=10000, version = "5.0", HIV = "h", Malaria = "h")
set.seed(123)
...
# Check that stillbirths are in IV5neonate
# table(IV5neonate$i104o, exclude = NULL)
# table(IV5neonate$i109o, exclude = NULL)
# table(IV5neonate$i110o, exclude = NULL)
# table(IV5neonate$i114o, exclude = NULL)
# test <- IV5neonate
# test$Stillbirth <- ifelse(test$i104o %in% c("N",".") & test$i109o %in% c("N",".") & test$i110o %in% c("N","."),1,0)
# 
# dim(IV5neonate)
# table(test$Stillbirth)
...
#                                 Nsim=10000, version = "5.0", HIV = "h", Malaria = "h")
# set.seed(123)
# codeVAInsilico.ch5_19y <- codeVA(IV5data.ch5_19y, data.type = "WHO2016", model = "InSilicoVA",
#                                 Nsim=10000, version = "5.0", HIV = "h", Malaria = "h")
