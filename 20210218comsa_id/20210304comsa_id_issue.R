...
# how does this affect using the VASA data?
Aug.COD.1to59months <- read.csv("~/Desktop/COMSA Analysis/Raw to CalibratedVA input pipeline/For/20210218comsa_id/Data/Aug.COD.1to59months.csv")
Mar4.eava_child_comsa <- read.csv("~/Desktop/COMSA Analysis/Raw to CalibratedVA input pipeline/For/20210218comsa_id/Data/Mar4.eava_child_comsa.csv")
Aug.VASA <- read_dta(file.path(file,"Data/eng_ext_child_VASA.dta"))
Mar4.VASA <- read_dta(file.path(file,"Data/20210304eng_ext_child_VASA.dta"))

names(Aug.VASA)[names(Aug.VASA) == 'comsa_id'] <- 'ID'
names(Mar4.VASA)[names(Mar4.VASA) == 'comsa_id'] <- 'ID'

test <- merge(Aug.VASA,Mar4.VASA,by="ID")
dim(Aug.VASA)
dim(Mar4.VASA)
dim(test)


# Aug VA with Aug VASA
test <- merge(Aug.COD.1to59months,Aug.VASA,by="ID")
dim(Aug.COD.1to59months)
dim(Aug.VASA)
dim(test)

# Mar VA with Mar VASA
test <- merge(Mar4.eava_child_comsa,Mar4.VASA,by="ID")
dim(Mar4.eava_child_comsa)
dim(Mar4.VASA)
dim(test)

# Aug VA with Mar VASA
test <- merge(Aug.COD.1to59months,Mar4.VASA,by="ID")
dim(Aug.COD.1to59months)
dim(Mar4.VASA)
dim(test)

# Mar VA with Aug VASA
test <- merge(Mar4.eava_child_comsa,Aug.VASA,by="ID")
dim(Mar4.eava_child_comsa)
dim(Aug.VASA)
dim(test)


# Aug VASA with Mar VASA
test <- merge(Aug.VASA,Mar4.VASA,by="ID")
dim(Aug.VASA)
dim(Mar4.VASA)
dim(test)
